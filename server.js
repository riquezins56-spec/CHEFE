    ...
    if(req.method==='GET'&&pathname==='/api/address-search'){
      ...
      try{
        ...
        const score=x=>{
          ...
          if(hintNeighborhood){const sim=searchSimilarity(hintNeighborhood,nb);n+=Math.round(sim*75);if(normalizeSearchText(nb)===normalizeSearchText(hintNeighborhood))n+=55}
          if(hintStreet&&hintNeighborhood&&normalizeSearchText(road)===normalizeSearchText(hintStreet)&&normalizeSearchText(nb)===normalizeSearchText(hintNeighborhood))n+=100;
        ...
        const out=all.map(x=>({...x,_score:score(x)})).sort((x,y)=>y._score-x._score).filter(x=>{
          ...
          const cityName=ad.city||ad.town||ad.municipality||ad.village||cityHint||'';const number=ad.house_number||'';const title=[road||nb,number&&('Nº '+number),nb&&road?nb:''].filter(Boolean).join(' — ');const detail=[cityName,stateHint||ad.state,ad.postcode].filter(Boolean).join(' • ');return {lat:Number(x.lat),lng:Number(x.lon),label:x.display_name,title:title||x.display_name,detail,address:ad,kind:road?'road':(nb?'neighborhood':'place'),road,neighborhood:nb};
        });
