...
const oldSettingsSubmit=$('#settingsForm').onsubmit;$('#settingsForm').onsubmit=async e=>{e.preventDefault();const payload={name:$('#sName').value.trim(),whatsapp:$('#sWhatsapp').value.trim(),deliveryMode:$('#sDeliveryMode').value,storeLat:$('#sStoreLat').value.trim(),storeLng:$('#sStoreLng').value.trim(),defaultEtaMinutes:Number($('#sEta').value)||0};if($('#sPassword').value)payload.adminPassword=$('#sPassword').value;await api('/api/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});$('#sPassword').value='';$('#settingsMsg').textContent='Configurações salvas.';state.settings={...state.settings,...payload};setTimeout(()=>$('#settingsMsg').textContent='',2500)};
